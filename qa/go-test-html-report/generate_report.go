...
func main() {
	app := cli.NewApp()
	app.Name = "go-test-html-report"
	app.Usage = "go-test-html-report generates a html report of go-test logs"
	app.Flags = []cli.Flag{
		&cli.StringFlag{
			Name:        "file,f",
			Usage:       "set the file of the go test json logs",
...
func report(c *cli.Context) error {
	...
	OS = strings.Split(strings.Split(fileName, "_")[1], ".")[0]
